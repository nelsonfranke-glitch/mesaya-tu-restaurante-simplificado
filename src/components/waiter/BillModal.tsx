...
import { X, Printer, CreditCard, CheckCircle2, Phone } from 'lucide-react';
...
const BillModal = ({ table, orders, total, onClose, onConfirm }: Props) => {
  const { markPaid, requestBill, currentUser } = useApp();
    ...
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
        <div className="bg-white rounded-2xl shadow-2xl p-8 text-center max-w-xs w-full">
          ...
          <h3 className="text-2xl font-bold text-gray-900">Encargado notificado</h3>
          <p className="text-gray-500 mt-2 text-sm">{table.name} — Esperando al encargado</p>
        </div>
      </div>
    );
  }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="bg-white text-black w-full max-w-sm rounded-2xl shadow-2xl flex flex-col max-h-[90vh]">
        ...
        <div className="flex items-center justify-between px-4 pt-4 print:hidden">
          <h3 className="font-bold text-lg text-gray-900">Cuenta — {table.name}</h3>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4 print:px-2 print:py-1" id="bill-ticket">
          <div className="text-center mb-3">
            <h2 className="font-bold text-2xl text-orange-600 print:text-black">MesaYa</h2>
